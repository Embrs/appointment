...
const initError = ref('');
const lastSpokenNumber = ref<number | null>(null);
...
const ActiveServiceId = computed(() => {
  ...
  // 自動挑：「當日有 WAITING 且 currentServing 最小」者；皆無 WAITING 時挑 ticketsTaken 最大
  const withWaiting = QueueServices.value.filter((s) => (s.waitingCount ?? 0) > 0);
  const pool = withWaiting.length > 0 ? withWaiting : QueueServices.value;
  if (pool.length === 0) return '';
  return [...pool].sort((a, b) =>
    (a.currentServing ?? 0) - (b.currentServing ?? 0)
  )[0]?.id ?? '';
});
...
const ApiLoad = async () => {
  ...
  try {
    ...
    // 把初始 snapshot 灌入 serviceMap，讓 WS 還沒連上時畫面也有資料
    for (const s of res.data.services) {
      if (s.bookingMode !== 'QUEUE') continue;
      const prev = queueStore.serviceMap[s.id];
      queueStore.serviceMap[s.id] = {
        serviceId: s.id,
        currentServing: s.currentServing ?? 0,
    ...
    // 鎖定初始 active service（避免 query 切換時 SSR/CSR 不一致）
    if (ActiveServiceId.value) {
      // 不立即播報切換到的當前號碼，但允許下次叫號廣播
      lastSpokenNumber.value = CurrentServing.value || null;
    }
  } finally {
    loading.value = false;
  }
...
// === 動畫 + TTS：watch currentServing 變化 ===
watch(CurrentServing, (next, prev) => {
  if (!next || next === prev) return;
  // 觸發動畫（重置 key 讓 CSS animation 重播）
  animateKey.value += 1;
  // TTS 廣播（同號碼不重播；切換服務後第一次的當前號碼不會在這裡觸發，因為 lastSpokenNumber 已先設為 CurrentServing）
  if (lastSpokenNumber.value === next) return;
  lastSpokenNumber.value = next;
  if (tts.isEnabled.value && ActiveService.value) {
    const lang = TtsLangMap[locale.value as 'zh' | 'en' | 'ja'] ?? TtsLangMap.zh;
    const text = t('display.tts.callPhrase', { number: next, serviceName: ActiveService.value.name });
    tts.Speak(text, lang);
  }
});
...
useHead({
  title: () => {
    ...
    const name = ActiveService.value?.name ?? merchantName.value ?? '';
  ...
  template(v-else)
    ...
    .PageDisplay__main
      ...
      section.PageDisplay__left
        ...
        .PageDisplay__serviceName(v-if="ActiveService") {{ ActiveService.name }}
        .PageDisplay__bigNumberWrap
          .PageDisplay__bigNumber(
            v-if="CurrentServing > 0"
            :key="animateKey"
            :class="{ 'PageDisplay__bigNumber--animate': animateKey > 0 }"
