...
系統 SHALL 提供 `GET /nuxt-api/auth/me`，以 Authorization Bearer JWT 識別當前身分，回傳所需顯示欄位。

#### Scenario: Admin 取得自身資訊

- **GIVEN** 有效 admin JWT
- **WHEN** 呼叫 me
- **THEN** 響應 200，`data` 含 `{ type:'admin', userName, userEmail }`

#### Scenario: Merchant 取得自身資訊（含商家名稱）

- **GIVEN** 有效 merchant JWT 且所屬 Merchant.status=ACTIVE
- **WHEN** 呼叫 me
- **THEN** 響應 200，`data` 含 `{ type:'merchant', userName, userEmail, merchantId, merchantName, role }`
